...
module Lutaml
  module Xsd
    ...
    class InteractiveBuilder
      attr_reader :entry_points, :options, :prompt
      attr_reader :resolved_mappings, :namespace_mappings, :pattern_mappings
      attr_reader :pending_schemas, :processed_schemas
      ...
      def extract_namespace_mappings(schema, path)
        ...
        unless namespace_exists?(schema.target_namespace)
      ...
      def handle_url_local_mode(url, namespace)
      ...
      def check_pattern_opportunity(location, resolved_path)
        ...
        if location_dir.include?("../") || location_dir.include?("/")
      ...
      def suggest_pattern(location, resolved_path)
        ...
        if prompt.yes?("Create this pattern mapping?")
          @pattern_mappings << {
            "from" => pattern_from,
            "to" => pattern_to,
            "pattern" => true
      ...
      def create_pattern_from_resolved(resolved_path, original_location)
      ...
      def derive_namespace_prefix(schema, path)
