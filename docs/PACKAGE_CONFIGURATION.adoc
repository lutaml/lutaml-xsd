...
Specifies the output path for the package file. This provides a convenient way to set the output path in the configuration file instead of using the `-o` command-line flag.

[source,yaml]
----
output_package: my-schemas.lxr
----

**Priority order** when determining output path:

1. CLI `-o` option (highest priority - overrides config)
2. `output_package` from configuration file
3. Default: `pkg/<name>.lxr`

.Using output_package field
[example]
====
[source,yaml]
----
# config.yaml
output_package: urban-function.lxr
...
[source,bash]
----
# Uses output_package from config
lutaml-xsd pkg build config.yaml

# Override with -o flag
lutaml-xsd pkg build config.yaml -o custom-name.lxr
----
====

==== `base_packages` (optional)

Array of base package configurations for package composition. Each entry can be a simple string (package path) or a full `BasePackageConfig` object with conflict resolution settings.
...
See link:PACKAGE_COMPOSITION.adoc[Package Composition Guide] for complete documentation on composing multiple packages.

**BasePackageConfig fields**:

* `package` (required) - Path to LXR package file
* `priority` (optional, default: 0) - Lower number = higher priority
* `conflict_resolution` (optional, default: "error") - Strategy: "keep", "override", or "error"
* `namespace_remapping` (optional) - Array of namespace URI transformations
* `exclude_schemas` (optional) - Glob patterns for schemas to exclude
* `include_only_schemas` (optional) - Glob patterns for schemas to include (whitelist)

==== `schema_location_mappings` (optional)

Array of schema location redirections for resolving imports and includes.
...
See link:SCHEMA_MAPPINGS.adoc[Schema Mapping Guide] for complete documentation.

==== `namespace_mappings` (optional)

Array of namespace prefix-to-URI mappings for qualified name resolution.
