...
=== Composed package structure

When you compose multiple packages using `base_packages`, the resulting package contains all schemas from all base packages merged together, not linked references.

**Key characteristics of composed packages**:

* **Schemas are merged** - All schemas from base packages are copied into the composed package
* **Single namespace registry** - All namespaces from all packages unified
* **Unified type index** - One index covering types from all packages
* **Metadata preservation** - Package metadata includes base package references
* **No external dependencies** - Composed package is self-contained

.Composed package metadata structure
[source,yaml]
----
# metadata.yaml in composed package
name: "Composed Schema Repository"
version: "1.0"
created_at: "2024-12-08T18:00:00+08:00"

# Base package references
base_packages:
  - package: "citygml-schemas.lxr"
    priority: 0
    conflict_resolution: "keep"

  - package: "iur-schemas.lxr"
    priority: 10
    conflict_resolution: "override"

# All schemas from both packages
files: 150
namespace_mappings: 12
schema_location_mappings: 25

# Statistics reflect merged content
statistics:
  total_schemas: 150
  total_types: 3500
  total_namespaces: 45
...
=== Inspecting composed packages

You can inspect composed packages to see which base packages were used and how schemas were merged.

.Inspecting a composed package
[example]
----
# Show package hierarchy with base packages
lutaml-xsd pkg inspect composed.lxr --show-tree

# Output shows composition structure:
Package: composed.lxr
├── Base Packages (2)
│   ├── citygml-schemas.lxr (priority: 0, strategy: keep)
│   │   └── 80 schemas, 1500 types
│   └── iur-schemas.lxr (priority: 10, strategy: override)
│       └── 70 schemas, 2000 types
├── Total Merged: 150 schemas, 3500 types
└── Namespaces: 45
----

.Viewing all schemas in composed package
[example]
----
# List all schemas (from all base packages)
lutaml-xsd pkg ls composed.lxr

# Shows unified list with namespace URIs
SCHEMAS (150 total)
┌─────────────────────────────────────────┬───────────────────┬──────────┬───────┐
│ Namespace URI                           │ File              │ Elements │ Types │
├─────────────────────────────────────────┼───────────────────┼──────────┼───────┤
│ http://www.opengis.net/citygml/2.0     │ cityGMLBase.xsd   │       45 │   120 │
│ https://www.geospatial.jp/iur/urf/3.2  │ urbanFunction.xsd │      245 │ 1,164 │
│ ... (ALL 150 schemas from all packages)│                   │          │       │
└─────────────────────────────────────────┴───────────────────┴──────────┴───────┘
----

**Important notes**:

* Composed packages are **snapshots** of the composition at build time
* Changes to base packages don't affect the composed package
* To update, rebuild the composed package from the configuration
* All type lookups work across all merged schemas seamlessly
