...
RSpec.describe Lutaml::Xsd::SchemaValidator do
  ...
  describe '#validate' do
    ...
    context 'with XSD 1.0 validator' do
      ...
      it 'raises error for invalid XML syntax' do
        ...
        end.to raise_error(Lutaml::Xsd::SchemaValidationError, /Invalid XML syntax/)
      end

      it 'raises error for non-XSD document' do
        expect do
          validator.validate(non_xsd_document)
        end.to raise_error(Lutaml::Xsd::SchemaValidationError, /Not a valid XSD schema/)
      end

      it 'raises error for wrong namespace' do
        expect do
          validator.validate(wrong_namespace)
        end.to raise_error(Lutaml::Xsd::SchemaValidationError, /invalid namespace/)
      end

      it 'raises error for missing namespace' do
        expect do
          validator.validate(no_namespace)
        end.to raise_error(Lutaml::Xsd::SchemaValidationError, /must be in the XML Schema namespace/)
      end

      it 'raises error for XSD 1.1 features (assert)' do
        expect do
          validator.validate(valid_xsd_1_1_with_assert)
        end.to raise_error(Lutaml::Xsd::SchemaValidationError, /XSD 1.1 features.*xs:assert/)
      end
    end
  ...
  describe 'integration with Lutaml::Xsd.parse' do
    let(:valid_schema) do
      <<~XSD
        <?xml version="1.0" encoding="UTF-8"?>
        <xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    it 'validates schema before parsing by default' do
      expect { Lutaml::Xsd.parse(valid_schema) }.not_to raise_error
    end
    ...
    it 'allows disabling validation' do
      ...
      end.not_to raise_error(Lutaml::Xsd::SchemaValidationError)
    end
  end
end
