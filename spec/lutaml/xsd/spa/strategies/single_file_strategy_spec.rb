...
RSpec.describe Lutaml::Xsd::Spa::Strategies::SingleFileStrategy do
  ...
  let(:config_loader) do
    ...
    File.write(File.join(config_dir, "ui_theme.yml"),
               "theme:\n  colors:\n    primary: '#000'\n")
    File.write(File.join(config_dir, "features.yml"),
               "features:\n  search:\n    enabled: true\n")
    File.write(File.join(config_dir, "templates.yml"),
               "templates:\n  layout: default\n")
  ...
  let(:renderer) do
    ...
    File.write(File.join(template_dir, "layout.html.liquid"),
               "<html><body>{{ content }}</body></html>")
    File.write(File.join(template_dir, "components", "schema_card.liquid"),
               "<div>{{ schema.name }}</div>")
    File.write(File.join(template_dir, "components", "_schema_detail.liquid"),
  ...
  describe "#generate" do
    ...
    it "renders main content" do
      content = renderer.render_partial("schema_card",
                                        { "schema" => serialized_data[:schemas].first })
  ...
  describe "#build_context" do
    ...
    it "builds context hash for template" do
      context = strategy.send(:build_context, serialized_data, theme, features,
                              templates_config)
