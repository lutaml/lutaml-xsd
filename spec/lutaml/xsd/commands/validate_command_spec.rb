...
RSpec.describe Lutaml::Xsd::Commands::ValidateCommand do
  ...
  describe "#run" do
    ...
    it "expands glob patterns" do
      ...
      allow(validator).to receive(:validate).and_return(double(valid?: true, errors: []))
  ...
  describe "output formats" do
    ...
    it "outputs text format by default" do
      expect { command.send(:output_results, results) }.to output(/Validation Results/).to_stdout
    end
    ...
    it "outputs JSON format when requested" do
      ...
      expect { json_command.send(:output_results, results) }.to output(/"summary"/).to_stdout
    end
    ...
    it "outputs YAML format when requested" do
      ...
      expect { yaml_command.send(:output_results, results) }.to output(/summary:/).to_stdout
    end
  end
  ...
  describe "exit codes" do
    ...
    it "exits with 0 for valid files" do
      ...
      expect { command.send(:exit_with_status, results) }.to raise_error(SystemExit) do |error|
        expect(error.status).to eq(0)
      end
    end
