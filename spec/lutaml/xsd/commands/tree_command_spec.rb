...
RSpec.describe Lutaml::Xsd::Commands::TreeCommand do
  ...
  after do
    File.delete(test_package_path) if File.exist?(test_package_path)
  end
  ...
  describe "#run" do
    ...
    it "includes package name in output" do
      ...
      expect { command.run }.to output(/#{Regexp.escape(package_name)}/).to_stdout
    end
    ...
    context "with flat format" do
      it "displays flat list" do
        ...
        expect { command.run }.to output(/schemas\//).to_stdout
      end
    end
