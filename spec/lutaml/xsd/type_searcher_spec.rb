  ...
  describe "#search" do
    context "when searching by name" do
      it "finds types with exact name match" do
        ...
        if results.any?
          ...
          expect(exact_match).not_to be_nil if results.any? { |r| r.local_name == "BuildingDetailsType" }
        end
      end
      ...
      it "finds types that start with the query" do
        ...
        if results.any?
          starts_with_match = results.find { |r| r.match_type == "name_starts_with" }
          expect(starts_with_match).not_to be_nil if results.any? { |r| r.local_name.start_with?("Building") }
        end
      end
      ...
      it "finds types that contain the query" do
        ...
        if results.any?
          ...
          expect(contains_match).not_to be_nil if results.any? { |r| r.local_name.include?("Type") }
        end
      end
    ...
    context "when searching by documentation" do
      ...
      it "returns empty array when no documentation matches" do
        results = searcher.search("zyxwvutsrqponmlkjihgfedcba", in_field: "documentation")
        expect(results).to be_empty
      end
    end
    ...
    context "when searching in both name and documentation" do
      it "searches in both fields by default" do
        ...
        if results.any?
          name_matches = results.select { |r| r.match_type.start_with?("name_") }
    ...
    context "with relevance scoring" do
      it "ranks exact name matches highest" do
        ...
        if results.any? { |r| r.local_name == "BuildingDetailsType" }
          exact_match = results.find { |r| r.local_name == "BuildingDetailsType" }
      ...
      it "ranks starts-with matches higher than contains" do
        ...
        starts_with_results = results.select { |r| r.match_type == "name_starts_with" }
        contains_results = results.select { |r| r.match_type == "name_contains" }

        if starts_with_results.any? && contains_results.any?
          expect(starts_with_results.first.relevance_score).to be > contains_results.first.relevance_score
        end
      end
      ...
      it "sorts by name when scores are equal" do
        ...
        results.group_by(&:relevance_score).each do |_score, group|
    ...
    it "respects repository namespace configuration" do
      ...
      results.each do |result|
        ...
        if result.namespace
          ...
          if prefix
            expect(result.qualified_name).to match(/^#{prefix}:/)
          end
        end
      end
    end
