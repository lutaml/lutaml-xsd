...
* <<rng-spa-documentation,RNG/RNC grammar documentation>> - Interactive HTML
  documentation for RELAX NG schemas with model tree view
  - Supports both `.rng` (XML) and `.rnc` (compact) syntax
  - YAML configuration for listing grammar files
  - Single-file mode for offline use
...
* <<cli-commands,Command-line interface>>
  ...
  - `spa` command for XSD documentation generation
  - `rng-spa` command for RNG/RNC grammar documentation generation
...
[[rng-spa-documentation]]
=== RNG-SPA command (RNG/RNC grammar documentation)

Generate interactive HTML documentation from RNG or RNC grammar files.
...
# Generate with separate asset files (requires HTTP server)
lutaml-xsd rng-spa config.yml --output docs.html --mode cdn

# Generate with custom title
lutaml-xsd rng-spa config.yml --output docs.html --title "My Grammar Docs"
----

==== Configuration file

The RNG-SPA command takes a YAML configuration file listing the RNG/RNC files
to document. File paths are resolved relative to the configuration file's
location.
...
`files`:: Array of RNG or RNC file paths to include in the documentation
(required). Both `.rng` (XML syntax) and `.rnc` (compact syntax) formats are
supported.

==== Frontend assets

The RNG-SPA frontend (`frontend-rng/dist/`) is gitignored. When using from git
(development), build the frontend before generating documentation:

[source,bash]
----
bundle exec rake build_frontend_rng
----

When installed as a gem, assets are pre-built and bundled — no build required.
