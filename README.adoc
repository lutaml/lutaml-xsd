...
- `is_any?` — Returns `true` if the object is an `Any` instance.
- `is_all?` — Returns `true` if the object is an `All` instance.
- `is_choice?` — Returns `true` if the object is a `Choice` instance.
- `is_element?` — Returns `true` if the object is an `Element` instance.
- `is_sequence?` — Returns `true` if the object is a `Sequence` instance.
- `is_attribute?` — Returns `true` if the object is an `Attribute` instance.
- `is_annotation?` — Returns `true` if the object is an `Annotation` instance.
- `is_simple_content?` — Returns `true` if the object is a `SimpleContent` instance.
- `is_attribute_group?` — Returns `true` if the object is an `AttributeGroup` instance.
