...
if type_index
  gml_types = type_index.select { |qname, _entry| qname.start_with?("{#{gml_ns}}") }
  ...
  if gml_types.empty?
    ...
    namespaces = type_index.keys.map { |qname| qname[/\{([^}]+)\}/, 1] }.compact.uniq
    namespaces.each do |ns|
      ns_types = type_index.select { |qname, _entry| qname.start_with?("{#{ns}}") }
