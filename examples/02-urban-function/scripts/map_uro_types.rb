...
def resolve_type_to_target(type_name, doc, namespaces)
  ...
  if elements.length == 1 && elements.first['ref']
    return { 'target' => elements.first['ref'] }
  elsif elements.length > 1
    element_names = elements.map { |el| el['ref'] || el['name'] || el['type'] }.join(', ')
    return { 'complex_target' => "Multiple elements in sequence: #{element_names}" }
  elsif elements.length == 1 && !elements.first['ref']
    element_info = elements.first['name'] || elements.first['type'] || 'unnamed'
    return { 'complex_target' => "Single element without ref: #{element_info}" }
  else
    return { 'complex_target' => "No elements in sequence or empty sequence" }
  end
end
...
def process_uro_generic_application_properties(doc, namespaces)
  ...
  elements.each do |element|
    ...
    if subst_group && subst_group.include?('_GenericApplicationPropertyOf')
...
puts "=" * 60
uro_results = process_uro_generic_application_properties(urban_object_doc, namespaces)

puts "\n" + "=" * 60
puts "Processing URF elements with uro: types..."
puts "=" * 60
urf_results = process_urf_uro_elements(urban_function_doc, urban_object_doc, namespaces)
...
output = {
  'elements' => {
    ...
    'urf' => urf_resolved_list.map { |r| { 'source' => r['source'], 'target' => r['target'] } }
  }
}
